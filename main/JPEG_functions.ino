...
int jpegRender(int xpos, int ypos) {
  ...
  while ( JpegDec.readSwappedBytes()) { // Swap byte order so the SPI buffer can be used
    ...
    {
#if 1
      // full size tiles ok.  part tiles drawn row by row
      int w = (win_w + 7) & ~7;  //ensure 8-byte alignment
      if (w == win_w) tft.pushImage(mcu_x, mcu_y, win_w, win_h, pImg);
      else {
          for (int i = 0; i < win_h; i++) {
              tft.pushImage(mcu_x, mcu_y + i, win_w, 1, pImg + w * i);
          }
      }
#elif 1
      ...
      tft.pushColors(pImg, win_w * win_h, false);
#elif 1
      tft.setAddrWindow(mcu_x, mcu_y, win_w, win_h);  //new Adafruit
